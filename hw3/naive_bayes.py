...
def train_mle_estimator(train_images, train_labels):
    ...
    temp = train_images.T @ train_labels
    for c in range(10):
        sum_col_t = np.sum(train_labels[:, c])
        pi_mle[c] = sum_col_t / N
        theta_mle[:, c] = temp[:, c] / sum_col_t
...
def train_map_estimator(train_images, train_labels):
    ...
    N, D = train_images.shape
    pi_map = np.zeros(10)
    theta_map = np.zeros((D, 10))

    temp = train_images.T @ train_labels
    for c in range(10):
        sum_col_t = np.sum(train_labels[:, c])
        pi_map[c] = sum_col_t / N
        theta_map[:, c] = (temp[:, c] + 2) / (sum_col_t + 4)
...
def log_likelihood(images, theta, pi):
    ...
    # YOU NEED TO WRITE THIS PART
    ones_mat1 = np.ones(images.shape)
    ones_mat2 = np.ones(theta.shape)
    temp = np.log(pi) + images @ np.log(theta) + (ones_mat1 - images) @ np.log(ones_mat2 - theta)
    log_like = temp - np.log(np.sum(np.exp(temp), axis=1))[:, np.newaxis]
    return log_like
...
def accuracy(log_like, labels):
    ...
    accuracy = np.sum(np.equal(c, pred)) / labels.shape[0]
    return accuracy
...
def image_sampler(theta, pi, num_images):
    ...
    for _ in range(num_images):
        c = np.random.choice(np.arange(10, dtype=int), p=pi)
        sampled_images[_] = np.random.binomial(1, p=theta[:, c])
